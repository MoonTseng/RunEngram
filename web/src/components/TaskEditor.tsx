...
export function TaskEditor({
  ...
  const save = async () => {
    ...
    if (!trimmedTitle || isSaving) {
      if (!trimmedTitle) setError(t("editor.titleRequired"));
      return;
    }
    try {
      if (isCreate) {
  ...
  const dialogLabel = isCreate
    ? `${t("editor.createDialog")} ${project.name}`
    : `${t("editor.editDialog")} ${currentTask.title}`;
        ...
        <header className="shrink-0 border-b border-[var(--tl-outline)] px-5 py-4 pr-12">
          <div className="flex min-w-0 items-start justify-between gap-3">
            <h3 className="font-bold text-base text-[var(--tl-ink)]">
              {isCreate
                ? `${t("editor.newTitle")} ${project.name}`
                : t("editor.editTitle")}
            </h3>
            {!isCreate && (
        ...
        <div data-task-editor-scroll-body className="min-h-0 flex-1 space-y-2.5 overflow-y-auto px-5 py-4">
          ...
          <div className="space-y-1">
            ...
            <div className="group relative">
              ...
              <button
                ...
                type="button"
                aria-label={t("editor.openMarkdown")}
                tabIndex={-1}
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            <label className="flex min-w-0 items-center gap-2 text-xs">
              <span className="shrink-0 text-[var(--tl-ink-muted)]">{t("editor.type")}</span>
              <select
              ...
              >
                <option value="feature">{typeLabel("feature")}</option>
                <option value="bug">{typeLabel("bug")}</option>
                <option value="docs">{typeLabel("docs")}</option>
              </select>
            </label>
            <label className="flex min-w-0 items-center gap-2 text-xs">
              <span className="shrink-0 text-[var(--tl-ink-muted)]">{t("editor.state")}</span>
              <select
                aria-label={t("editor.state")}
                className="min-w-0 flex-1 border border-[var(--tl-outline)] rounded-md px-2 py-1"
                value={state}
                onChange={(e) => {
            ...
            <label className="flex min-w-0 items-center gap-2 text-xs">
              <span className="shrink-0 text-[var(--tl-ink-muted)]">{t("editor.priority")}</span>
              <input
...
function ClaimSection({ task }: { task: Task }) {
  ...
  const claimVerb = task.state === "done" ? t("claim.worked") : t("claim.working");
  const claimElapsed = task.claimed_at
    ? task.state === "done"
      ? task.completed_at
        ? formatElapsedTime(task.claimed_at, task.completed_at, locale)
        : ""
      : formatElapsedTime(task.claimed_at, undefined, locale)
    : "";
  const claimedAt = task.claimed_at
    ? new Date(task.claimed_at).toLocaleString(locale)
    : t("claim.unknown");
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-medium text-[var(--tl-ink-muted)]">{t("claim.label")}</p>
      ...
      <div className="mt-2 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
        <div
          ...
          title={task.claimed_at ? `${t("claim.claimed")} ${claimedAt}` : undefined}
        >
        ...
        <div className="flex min-w-0 items-center gap-1.5 text-[var(--tl-ink-muted)]">
          ...
          <span className="shrink-0">{t("claim.leaseExpires")}</span>
...
function LabelSection({
  ...
  return (
    <div className="border-t border-[var(--tl-outline)] pt-3 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-medium text-[var(--tl-ink-muted)]">{t("labels.label")}</p>
      </div>
...
function ImageSection({
  ...
  return (
    <div className="border-t border-[var(--tl-outline)] pt-3 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-medium text-[var(--tl-ink-muted)]">{t("images.label")}</p>
        <label
          className={
        ...
        >
          ...
          <span>{upload.isPending ? t("images.uploading") : t("images.upload")}</span>
          <input
            ref={inputRef}
            aria-label={t("images.attachment")}
            className="sr-only"
            type="file"
            accept="image/*"
      ...
      {displayedImages.length > 0 ? (
        <ul className="space-y-1">
          {displayedImages.map((image) => (
            ...
            >
              <button
                type="button"
                aria-label={`${t("images.view")} ${image.filename}`}
              ...
              >
                ...
                <span className="text-[var(--tl-ink-faint)] shrink-0">
                  {image.mime_type || t("images.unknown")}
                </span>
              ...
              <button
                type="button"
                aria-label={`${t("images.delete")} ${image.filename}`}
        ...
        <p className="text-xs text-[var(--tl-ink-faint)]">{t("images.empty")}</p>
      )}
      {error && <p className="text-xs text-[var(--tl-rust)]">{error}</p>}
      {previewImage && (
        <div
...
function DocSection({
  ...
  return (
    <div className="border-t border-[var(--tl-outline)] pt-3 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-medium text-[var(--tl-ink-muted)]">{t("docs.label")}</p>
        {!disabled && (
          <button
            type="button"
            aria-label={t("docs.add")}
      ...
      {disabled ? (
        <p className="text-xs text-[var(--tl-ink-faint)]">{t("docs.createFirst")}</p>
      ) : docs.length > 0 ? (
        <ul className="space-y-1">
          {docs.map((doc) => (
            <li
            ...
            >
              <button
                type="button"
                aria-label={`${t("docs.open")} ${doc.title}`}
                className="flex flex-1 min-w-0 items-center gap-2 text-left"
              ...
              <button
                type="button"
                aria-label={`${t("docs.delete")} ${doc.title}`}
                className="h-5 w-5 shrink-0 rounded text-[var(--tl-ink-faint)] opacity-0 group-hover:opacity-100 focus:opacity-100 hover:bg-[var(--tl-rust-soft)] hover:text-[var(--tl-rust)] flex items-center justify-center disabled:opacity-50"
        ...
        <p className="text-xs text-[var(--tl-ink-faint)]">{t("docs.empty")}</p>
      )}
      {error && <p className="text-xs text-[var(--tl-rust)]">{error}</p>}
      {dialog && (
        <Suspense
...
function LinkSection({
  ...
  return (
    <div className="border-t border-[var(--tl-outline)] pt-3 space-y-2">
      <p className="text-xs font-medium text-[var(--tl-ink-muted)]">{t("links.label")}</p>
        ...
        <ul className="space-y-1">
          {displayedLinks.map((l) => (
            <li key={l.id} className="text-xs flex items-center gap-2 group">
              ...
              <button
                type="button"
                aria-label={`${t("links.remove")} ${l.label || l.url}`}
      ...
      <div className="flex items-stretch gap-2">
        ...
        <input
          ...
          placeholder={t("links.optionalLabel")}
          value={label}
          disabled={disabled}
...
function DependsSection({
  ...
  return (
    <div className="border-t border-[var(--tl-outline)] pt-3 space-y-2">
      <p className="text-xs font-medium text-[var(--tl-ink-muted)]">
      ...
      {selectedDependencyIds.length > 0 ? (
            ...
            return (
              <li key={id} className="text-xs flex items-center gap-2 group">
                ...
                {dep ? (
                  ...
                  <span className="text-[var(--tl-ink-faint)] italic flex-1">
                    {t("dependencies.deleted")}
                  </span>
                )}
                <button
                  type="button"
        ...
        <p className="text-xs text-[var(--tl-ink-faint)]">{t("dependencies.empty")}</p>
      )}
      <select
        aria-label={t("dependencies.add")}
      ...
      >
        <option value="">{t("dependencies.addPlaceholder")}</option>
        {candidates.map((t) => (

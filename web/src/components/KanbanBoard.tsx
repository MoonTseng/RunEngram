...
const SORT_OPTIONS: Array<{ id: ColumnSortMode; labelKey: TranslationKey }> = [
  { id: "execution", labelKey: "sort.execution" },
  { id: "priority", labelKey: "sort.priority" },
  { id: "created", labelKey: "sort.created" },
  { id: "updated", labelKey: "sort.updated" },
];
...
function Column({
  ...
  const { locale, stateLabel: getStateLabel, t } = useI18n();
  const { setNodeRef, isOver } = useDroppable({ id: state });
  const stateLabel = getStateLabel(state);
  const sortLabel =
    locale === "zh-CN" ? `排序：${stateLabel}` : `Sort ${stateLabel} tasks`;

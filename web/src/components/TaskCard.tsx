...
export function TaskCard({
  ...
  const claimTitle = claimOwner
    ? buildClaimTitle(task, claimOwner, claimVerb, claimElapsed, locale, t)
    : undefined;
  ...
  return (
    <div
      ...
      {...(overlay ? {} : attributes)}
      {...(overlay ? {} : listeners)}
      aria-label={overlay ? undefined : `${t("card.open")} ${task.title}`}
    ...
    >
      <div className="min-w-0">
        ...
        <div className="mt-1.5 flex max-h-[42px] min-w-0 flex-wrap items-start gap-1 overflow-hidden">
          <span
            ...
            title={`${t("card.priority")} ${task.priority}`}
          >
            p {task.priority}
          </span>
          {dependencyCount > 0 && (
            <span
              ...
              title={
                isBlocked ? t("card.blocked") : t("card.dependenciesDone")
              }
            >
              {t("card.dependencies")} {dependencyCount}
            </span>
          )}
          ...
          {hiddenLabelCount > 0 && (
            <span
              ...
              title={`${hiddenLabelCount} ${t("card.moreLabels")}`}
            >
              +{hiddenLabelCount}
            </span>
          )}
      ...
      {claimOwner && (
        <div className="mt-1.5 min-w-0">
          <span
            aria-label={
              claimElapsed
                ? `${t("card.claimedBy")} ${claimOwner}, ${claimVerb} ${claimElapsed}`
                : `${t("card.claimedBy")} ${claimOwner}`
            }
      ...
      <div className="mt-1 flex min-w-0 justify-end">
        <button
          type="button"
          aria-label={`${t("card.viewHistory")} ${task.title}`}
          ...
          title={new Date(task.updated_at).toLocaleString(locale)}
          onPointerDown={(event) => {
            pointerStart.current = null;
            event.stopPropagation();
          }}
        ...
        >
          {formatRelativeTime(task.updated_at, undefined, locale)}
        </button>
      </div>
    </div>
  );
...
function buildClaimTitle(
  ...
  const parts = [`${t("card.owner")}: ${owner}`];
  if (claimElapsed) {
    const titleVerb =
      locale === "en-US"
        ? claimVerb.charAt(0).toUpperCase() + claimVerb.slice(1)
        : claimVerb;
    parts.push(`${titleVerb} ${claimElapsed}`);
  }
  if (task.claimed_at) {
    parts.push(
      `${t("claim.claimed")} ${formatRelativeTime(task.claimed_at, undefined, locale)} (${new Date(
        task.claimed_at
  ...
  if (task.lease_expires_at) {
    const leaseLabel =
      locale === "en-US" ? "Lease expires" : t("claim.leaseExpires");
    parts.push(
      `${leaseLabel} ${new Date(task.lease_expires_at).toLocaleString(locale)}`
    );
  }
  return parts.join(" · ");
}

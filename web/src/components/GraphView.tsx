...
export function GraphView({ project }: Props) {
  ...
  const showMutationError = useCallback((err: unknown) => {
    setError(err instanceof Error ? err.message : t("graph.updateFailed"));
      ...
      {selected && (
        <EdgeLabelRenderer>
          <button
            ...
            aria-label={
              locale === "zh-CN"
                ? `${t("graph.deleteDependency")} ${sourceTitle} → ${targetTitle}`
                : `${t("graph.deleteDependency")} ${sourceTitle} to ${targetTitle}`
            }
            title={t("graph.deleteDependency")}
...
function TaskNode({
  ...
  const { stateLabel, typeLabel } = useI18n();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] uppercase text-[var(--tl-ink-muted)]">
          {typeLabel(task.type)}
        </span>

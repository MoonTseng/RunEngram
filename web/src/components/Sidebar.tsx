  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          {creating ? t("actions.cancel") : t("sidebar.newProject")}
        </button>
      </div>
      {creating && (
        <form
      ...
      <nav className="flex-1 overflow-auto">
        {projects.isLoading && (
          <p className="text-sm text-[var(--tl-ink-muted)]">{t("sidebar.loading")}</p>
        )}
        {projects.error && (
          <p className="text-sm text-[var(--tl-rust)]">
            {t("sidebar.failed")} {(projects.error as Error).message}
          </p>
        )}
        {projects.data?.length === 0 && (
          <p className="text-xs text-[var(--tl-ink-muted)]">{t("sidebar.empty")}</p>
        )}

  ...
  return (
    ...
    >
      <section
        role="dialog"
        aria-modal="true"
        aria-label={`${t("history.dialog")} ${task.title}`}
      ...
      >
        <header className="flex items-center gap-3 border-b border-[var(--tl-outline)] px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-semibold text-[var(--tl-ink)]">
              {t("history.title")}
            </h3>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {isLoading ? (
            <p className="py-10 text-center text-sm text-[var(--tl-ink-muted)]">
              {t("history.loading")}
            </p>
          ) : error ? (
                ...
                >
                  <div className="flex min-w-0 flex-wrap items-start gap-x-2 gap-y-1">
                    ...
                    <time
                      ...
                      title={new Date(event.created_at).toLocaleString(locale)}
                    >
      ...
      {Object.entries(changes).map(([field, change]) => (
        <div key={field} className="min-w-0">
          ...
          {change.changed && change.before === undefined && change.after === undefined ? (
            <dd className="mt-0.5 text-xs text-[var(--tl-ink)]">
              {t("history.contentChanged")}
            </dd>
          ) : (
            <dd className="mt-1 grid min-w-0 grid-cols-2 gap-2 max-sm:grid-cols-1">
              <ChangeValue label={t("history.before")} value={change.before} />
              <ChangeValue label={t("history.after")} value={change.after} />
            </dd>
          )}
        </div>
      ))}
